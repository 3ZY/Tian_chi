...
for site in site_points:
	#初始化网点耗时
	site_cost[site]=0xffffffffffffffff
...
site_sum_shop_cost={}#网点与商户的距离总和
for site in site_points:
	site_sum_shop_cost[site]=0
	cost=[dist(points[site][0],points[site][1],points[shop][0],points[shop][1]) for shop in shop_points]
	site_sum_shop_cost[site]+=sum(cost)
site_w={} #网点的惩罚
for site in site_points:
	site_w[site]=(site_sum_shop_cost[site]/1000)**2.2
...
for _ in range(C_man):
	...
	for site,paths_cost in site_paths_cost.items():
		...
		#惩罚
		total_cost=total_cost*site_w[site]
...
for oid,order in sorted(e_orders.items(),key=lambda x:x[1][2]):
	...
	for man,paths in man_paths.items():
		...
		if order[2]>(man_cost[man]+distance):
			...
			add_cost+=(man_cost[man]+distance-order[2])*C_penalty_tw*7.3
		if (man_cost[man]+distance+tmp) > order[3]:
			add_cost+=(man_cost[man]+distance+tmp-order[3])*C_penalty_tw*7.3
	...
	for next_oid in e_spot_oids[spot]:#相同商点，配送点下，时间应该是升序的
		...
		#此处判断可设定
		if (man_cost[best_man] >= e_orders[next_oid][2]-9):
			now_oid=next_oid
			used_oid.add(now_oid)
			oids.append(now_oid)
			continue
		#此处判断可设定
		if (e_orders[next_oid][2]-e_orders[now_oid][2]) <=9:
			now_oid=next_oid
			used_oid.add(now_oid)
			oids.append(now_oid)
